          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[32px] md:gap-[40px]">
            {galleryImages.slice(0, -2).map((img, idx) => (
              <Reveal key={idx} width="100%" delay={idx * 0.2}>
                <div
                  className={`w-full aspect-[3/4] bg-gray-100 rounded-[20px] overflow-hidden ${
                    idx === 1 ? "md:mt-24" : ""
                  }`}
                >
                  <img
        ...
        <section className="w-full bg-[#F5F5F0] py-[100px]">
          <div className="w-full max-w-content px-6 md:px-10 mx-auto">
            <div className="flex justify-between items-baseline mb-12">
              <h2 className="font-sans font-black text-[24px] uppercase tracking-tight text-black">
                More Work
              </h2>
              <Link
                to="/work"
                className="text-[13px] font-medium text-black border-b border-black/20 pb-0.5 hover:border-black transition-colors"
              >
                View all
              </Link>
            </div>
